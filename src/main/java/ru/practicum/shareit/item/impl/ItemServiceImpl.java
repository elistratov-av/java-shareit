...
public class ItemServiceImpl implements ItemService {
    ...
    private List<ItemInfoDto> mergeWithItems(List<Item> items, List<Booking> bookings, List<Comment> comments) {
        ...
        for (Booking b : bookings) {
            bookingsMapByItemId.putIfAbsent(b.getItem().getId(), new ArrayList<>()).add(b);
        }
        Map<Long, List<Comment>> commentsMapByItemId = new HashMap<>();
        for (Comment c : comments) {
            commentsMapByItemId.putIfAbsent(c.getItem().getId(), new ArrayList<>()).add(c);
        }
    ...
    public ItemDto add(ItemCreateDto newItem, long userId) {
        User owner = userRepository.findById(userId)
                .orElseThrow(() -> new NotFoundException("Владелец с id = " + userId + " не найден"));
        Item item = itemRepository.save(itemMapper.toItem(newItem, owner));
